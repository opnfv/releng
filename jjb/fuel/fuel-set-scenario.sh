...
# or defaulting to the simplest HA scenario for baremetal via topic name.
#
...
#
# Patterns to be searched in topic name:
#   baremetal
...
function set_scenario() {
    ...
    if [ -z "${DEPLOY_SCENARIO}" ]; then
        if [[ "$GERRIT_TOPIC" =~ baremetal ]] && [[ "$JOB_NAME" =~ baremetal ]]; then

...
# TODO(trozet) insert new ODL
if [ -n "$NETVIRT_ARTIFACT" ]; then
  echo "New OpenDaylight build detected, will replace existing OpenDaylight"
...
while [ "$counter" -le 10 ]; do
  if nova list; then
    echo "Overcloud Nova is up...Checking if OpenDaylight NetVirt is up..."
    ...
    echo "Nova not yet up, try ${counter}"
  fi
  counter=$((counter+1))
  sleep 60
done

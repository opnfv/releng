...
get_pod_file() {
    ...
    check_file_exists ${POD} || get_pod_file_with_scripts

    check_file_exists ${POD}
    if [[ $? == 0 ]]; then
        change_pod_file_process_info
    else
        echo "ERROR: cannot find file ${POD}. Please check if it is existing."
        sudo ls -al ${DOVETAIL_CONFIG}
        exit 1
    fi

...
# we need to fetch the change and look at the patchset to find out the scenario instead
# of hardcoding scenario per project for the projects that develop multiple scenarios such as fds.
...
if [[ $(echo $DEPLOY_SCENARIO | wc -w) != 1 ]]; then
    echo "Change touches multiple scenarios!"
    echo "XCI doesn't support testing for multiple scenarios at one go currently."

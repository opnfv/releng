...
cd /opt/bifrost && sudo git checkout $OPENSTACK_BIFROST_VERSION
echo "xci: using $(git show --oneline -s --pretty=format:'%h - %s (%cr) <%an>')"

sudo git clone --quiet https://gerrit.opnfv.org/gerrit/releng /opt/releng
cd /opt/releng && sudo git checkout $OPNFV_RELENG_VERSION
echo "xci: using $(git show --oneline -s --pretty=format:'%h - %s (%cr) <%an>')"

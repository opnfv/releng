...
if [[ ${INSTALLER_TYPE} == 'compass' ]]; then
    ...
    echo "KUBECONFIG=/root/.kube/config" >> $rc_file
    echo "KUBERNETES_PROVIDER=local" >> $rc_file
    KUBE_MASTER_URL = $(cat ${HOME}/admin.conf|grep server| awk '{print $2}')
    echo "KUBE_MASTER_URL=$KUBE_MASTER_URL" >> $rc_file
    KUBE_MASTER_IP = $(echo $KUBE_MASTER_URL|awk -F'https://|:[0-9]+' '$0=$2')
    echo "KUBE_MASTER_IP=$KUBE_MASTER_IP" >> $rc_file
else
...
cmd="docker run --rm --privileged=true --env-file $rc_file ${volumes} ${FUNCTEST_IMAGE} /bin/bash -c '${cmd_opt}'"

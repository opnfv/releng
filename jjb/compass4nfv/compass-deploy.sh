...
if [[ "${DEPLOY_SCENARIO}" =~ "-ocl" ]]
    export NET_CONF_FILE=network_ocl.yml
else
    export NET_CONF_FILE=network.yml
fi

if [[ "$NODE_NAME" =~ "-virtual" ]]; then
    export NETWORK_CONF=$CONFDIR/vm_environment/$NODE_NAME/${NETWORK_CONF}
    export DHA_CONF=$CONFDIR/vm_environment/${DEPLOY_SCENARIO}.yml
else
    export INSTALL_NIC=eth1
    export NETWORK_CONF=$CONFDIR/hardware_environment/$NODE_NAME/${NETWORK_CONF}

...
if [[ "$1" == "--update_latest_stable" ]]; then
    echo "Pulling $DOCKER_REPO_NAME:$2 ..."
    docker pull $DOCKER_REPO_NAME:$2
    if [[ $? -ne 0 ]]; then
        echo "ERROR: The image $DOCKER_REPO_NAME with tag $2 does not exist."
        exit 1
    fi
    docker tag $DOCKER_REPO_NAME:$2 $DOCKER_REPO_NAME:latest_stable

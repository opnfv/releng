...
for release_file in $RELEASE_FILES; do
    ...
    while read -r repo branch ref; do
        ...
        branches="$(git ls-remote --exit-code "https://gerrit.opnfv.org/gerrit/$repo.git" "refs/heads/$branch")"

        if ! [ -z "$branches" ]; then
            echo "$ref is already assigned to $branches $branch"
        else
            #echo for test mode
            echo "ssh -f -p 29418 gerrit.opnfv.org gerrit create-branch "$repo" "$branch" "$ref""
        fi

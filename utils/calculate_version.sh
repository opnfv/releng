...
STORAGE_TYPES=(docker iso artifact)
TYPE=""
NAME=""
...
if [ -z "$BASE_VERSION" ]; then
    error "Base version must be specified as environment variable. Ex.: BASE_VERSION='brahmaputra.0'
fi
...
function docker_version() {
    ...
    #ex: tag_json= "name": "brahmaputra.0.2",
    if [ "${tag_json}" == "" ]; then
    ...
    tag_new=${BASE_VERSION}.${tag_new_version}
    echo $tag_new
}

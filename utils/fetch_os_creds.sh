...
if [ "$installer_type" == "fuel" ]; then
    ...
    sshpass -p vagrant ssh $ssh_options root@${installer_ip} 'sshpass -p Op3nStack ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@oscontroller1.opnfv.com "ls"'
    if [ $? -ne 0 ]; then
    ...
    sshpass -p vagrant ssh $ssh_options root@${installer_ip} 'sshpass -p Op3nStack scp -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@oscontroller1.opnfv.com:~/keystonerc_admin .'
    sshpass -p vagrant scp $ssh_options root@${installer_ip}:~/keystonerc_admin $dest_path

    #This file contains the mgmt keystone API, we need the public one for our rc file
    public_ip=$(sshpass -p vagrant ssh $ssh_options root@${installer_ip} 'sshpass -p Op3nStack ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@oscontroller1.opnfv.com "source keystonerc_admin;keystone endpoint-list"' | grep http | head -1 | cut -d '|' -f 4 | sed 's/ //g')

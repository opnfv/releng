...
if [ "$installer_type" == "fuel" ]; then
    ...
    sshpass -p r00tme ssh 2>/dev/null $ssh_options root@${installer_ip} 'scp ${controller_ip}:/root/openrc .' &> /dev/null
    ...
    public_ip=$(sshpass -p r00tme ssh $ssh_options root@${installer_ip} \
        'ssh ${controller_ip} "source openrc; keystone endpoint-list"' | grep http | head -1 | cut -d '|' -f 4 | sed 's/\([0-9]\):[0-9]*.*/\1/' | sed 's/ //g') &> /dev/null
    ...
    sshpass -p vagrant ssh $ssh_options root@${installer_ip} 'sshpass -p Op3nStack ssh $ssh_options root@oscontroller1.opnfv.com "ls"' &> /dev/null
    if [ $? -ne 0 ]; then
    ...
    sshpass -p vagrant ssh $ssh_options root@${installer_ip} 'sshpass -p Op3nStack scp $ssh_options root@oscontroller1.opnfv.com:~/keystonerc_admin .' &> /dev/null
    ...
    public_ip=$(sshpass -p vagrant ssh $ssh_options root@${installer_ip} 'sshpass -p Op3nStack ssh $ssh_options root@oscontroller1.opnfv.com \
        "source keystonerc_admin;keystone endpoint-list"' | grep http | head -1 | cut -d '|' -f 4 | sed 's/ //g') &> /dev/null

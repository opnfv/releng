...
from common.constants import APPLICATION_JSON, HTTP_BAD_REQUEST, \
...
class TestProjectHandler(GenericApiHandler):
    ...
    def delete(self, project_name):
        ...
        mongo_dict = yield self.db.test_cases.find_one({'project_name': project_name})
        test_project = TestProject.testproject_from_dict(mongo_dict)
        if test_project is None:
            raise HTTPError(HTTP_NOT_FOUND,
                            "{} could not be found as a project to be deleted".format(project_name))

        # just delete it, or maybe save it elsewhere in a future
        res = yield self.db.test_projects.remove({'project_name': project_name})
        print res
...
class TestCasesHandler(GenericApiHandler):
    ...
    def put(self, project_name, case_name):
        ...
        if test_case is None:
            raise HTTPError(HTTP_NOT_FOUND,
                            "{} could not be found as a {} test case to be updated".format(case_name, project_name))

        new_name = self.json_args.get("name")
        new_project_name = self.json_args.get("project_name")
        new_description = self.json_args.get("description")

        # check if there is not an existing test case with the name provided in the json payload
    ...
    def delete(self, project_name, case_name):
        ...
        if test_project is None:
            raise HTTPError(HTTP_NOT_FOUND,
                            "{}/{} could not be found as a case to be deleted".format(project_name, case_name))
...
class TestResultsHandler(GenericApiHandler):
    ...
    def get(self, result_id=None):
        ...
        GET /results/project=functest&case=keystone.catalog&pod=1 => get results with optional filters
        """
    ...
    def post(self):
        ...
        if not (mongo_dict is None):
            raise HTTPError(HTTP_NOT_FOUND,
                            "Could not find project [{}] ".format(self.json_args.get("project_name")))

        # check for case
        mongo_dict = yield self.db.test_cases.find_one({"name": self.json_args.get("case_name")})
        if not (mongo_dict is None):
            raise HTTPError(HTTP_NOT_FOUND,
                            "Could not find case [{}] ".format(self.json_args.get("case_name")))

        # check for pod
        mongo_dict = yield self.db.pod.find_one({"_id": self.json_args.get("pod_id")})
        if not (mongo_dict is None):
            raise HTTPError(HTTP_NOT_FOUND,
                            "Could not find POD [{}] ".format(self.json_args.get("pod_id")))

...
def _get_results(db_url, testcase):
    ...
    datajson = data.json()
    print len(datajson['test_results'])
    #Ignoring the first test result
    for x in range(1, len(datajson['test_results'])):

        rawresults = datajson['test_results'][x]['details']
        index = json.loads(str(rawresults))
        resultarray[str(datajson['test_results'][x]['creation_date'])]=index['index']

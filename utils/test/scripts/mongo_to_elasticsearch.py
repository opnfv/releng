...
def publish_document(document, es_creds, to):
    status, data = shared_utils.publish_json(document, to, es_creds)
    if status > 300:
        logger.error('Publish record[{}] failed, due to [{}]'
                    .format(document, json.loads(data)['error']['reason']))


def publish_nonexist_documents(elastic_docs, to, es_creds):
    try:
...
if __name__ == '__main__':
    ...
    elastic_docs = shared_utils.get_elastic_docs(base_elastic_url, es_creds, days)
    logger.info('number of hits in elasticsearch for now-{}d: {}'.format(days, len(elastic_docs)))
    publish_nonexist_documents(elastic_docs, to, es_creds)

    ...
    def wrapper(self, *args, **kwargs):
        ...
        if ticket:
            ...
            login_user = {
                ...
                'fullname': attributes.get('field_lf_full_name'),
                'groups': ['opnfv-testapi-users'] + attributes.get('group', [])
            }
            db_user = yield dbapi.db_find_one('users', {'user': user})
            if not db_user:
                dbapi.db_save('users', login_user)
            else:
                dbapi.db_update('users', {'user': user}, login_user)

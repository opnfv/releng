...
class TestScenarioUpdate(TestScenarioBase):
    ...
    def test_changeOwner(self):
        new_scenario = deepcopy(self.req_d)
        new_scenario['installers'][0]['versions'][0]['owner'] = 'lucy'

        update_req = models.ScenarioUpdateRequest(field='owner',
                                                  op='update',
                                                  locate={'installer': 'apex',
                                                          'version': 'master'},
                                                  term={'owner': 'lucy'})
    ...
    def test_addProject(self):
        add = models.ScenarioProject(project='qtip')
        new_scenario = deepcopy(self.req_d)
        new_scenario['installers'][0]['versions'][0]['projects'].append(
            add.format())
        update_req = models.ScenarioUpdateRequest(field='project',
                                                  op='add',
                                                  locate={'installer': 'apex',
                                                          'version': 'master'},
                                                  term=add.format())
    ...
    def test_deleteProject(self):
        new_scenario = deepcopy(self.req_d)
        new_scenario['installers'][0]['versions'][0]['projects'] = filter(
            lambda f: f['project'] != 'functest',
            new_scenario['installers'][0]['versions'][0]['projects'])
    ...
    def test_addCustoms(self):
        ...
        new_scenario = deepcopy(self.req_d)
        projects = new_scenario['installers'][0]['versions'][0]['projects']
        functest = filter(lambda f: f['project'] == 'functest', projects)[0]
        functest['customs'] = ['healthcheck', 'odl', 'parser', 'vping_ssh']
        update_req = models.ScenarioUpdateRequest(field='customs',
                                                  op='add',
                                                  locate={
                                                      'installer': 'apex',
                                                      'version': 'master',
    ...
    def test_addScore(self):
        ...
        update_req = models.ScenarioUpdateRequest(field='score',
                                                  op='add',
                                                  locate={
                                                      'installer': 'apex',
                                                      'version': 'master',
    ...
    def test_addTi(self):
        ...
        update_req = models.ScenarioUpdateRequest(field='trust_indicator',
                                                  op='add',
                                                  locate={
                                                      'installer': 'apex',
                                                      'version': 'master',

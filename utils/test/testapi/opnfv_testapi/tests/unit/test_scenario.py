...
class TestScenarioAddInstaller(TestScenarioBase):
    ...
    def _add(expected, e_code, e_message):
        def __add(add):
            @functools.wraps(add)
            def wrap(self):
    ...
    @_add('_noScenario',
          httplib.FORBIDDEN,
          message.not_found('scenario', 'noScenario'))
    ...
    def test_noName(self):
        req = deepcopy(self.req_scenario)
        req['installer'] = None
        return req, self.scenario

    @_add('_exist',
          httplib.FORBIDDEN,
          message.exist('installer', 'apex'))
    def test_exist(self):

...
if [[ -e "$file_of_jobs_to_delete" ]]; then
  ...
  for job in $(cat "$file_of_jobs_to_delete"); do
    if [[ $test_mode == true ]]; then
      echo "TEST MODE! curl -X POST "https://$username:$password@build.opnfv.org/ci/job/${job}/doDelete -H $crumb""
    else
      echo "RUNNING! curl -X POST "https://$username:$password@build.opnfv.org/ci/job/${job}/doDelete -H $crumb""
      curl -X POST "https://$username:$password@build.opnfv.org/ci/job/${job}/doDelete -H "$crumb""
    fi
   done
  ...
  for job in $(echo "$jobs" | tr "\n" " "); do
    if [[ $test_mode == true ]]; then
      echo "TEST MODE ONLY! curl -X POST "https://$username:$password@build.opnfv.org/ci/job/${job}/doDelete -H $crumb""
    else
      echo "RUNNING! curl -X POST "https://$username:$password@build.opnfv.org/ci/job/${job}/doDelete -H "$crumb"""
      curl -X POST "https://$username:$password@build.opnfv.org/ci/job/${job}/doDelete -H $crumb"
    fi

...
def create_branch(api, branch):
    ...
    # created at HEAD
    try:
        request = api.get("/projects/%(project)s/commits/%(commit)s" % arguments)
        logging.debug(request)
        #response = request[1]
        logging.debug("Commit exists: %(commit)s" % arguments)
    except RequestException as err:
        arguments['branch'] = unquote(arguments['branch'])
        if err.response.status_code in [404]:
            logging.warn("Commit %(commit)s for %(project)s:%(branch)s does"
    ...
    try:
        request = api.put("/projects/%(project)s/branches/%(branch)s" % arguments,
                          branch_data, headers=headers)
        resp_json = request[0]
        response = request[1]
        logging.info("Branch %(branch)s for %(project)s successfully"
                     "created" % arguments),
    except RequestException as err:

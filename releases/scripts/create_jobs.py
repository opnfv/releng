...
def main():
    ...
    logger.info("Jobs already exists for %s in files: %s", arg['project'], ', '.join(skipped_files))
    # Exit if there are not jobs to create
    if not job_files:
        return
    logger.info("Creating Jenkins Jobs for %s in files: %s", arg['project'], ', '.join(job_files))

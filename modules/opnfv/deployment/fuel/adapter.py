...
class FuelAdapter(manager.DeploymentHandler):
    ...
    def get_nodes(self, options=None):
        ...
        _map = {
            manager.Role.INSTALLER: 'salt:master:enabled',
            manager.Role.CONTROLLER: 'nova:controller:enabled',
            manager.Role.COMPUTE: 'nova:compute:enabled',
            manager.Role.ODL: 'opendaylight:server:enabled',
            manager.Role.ONOS: 'onos:server:enabled'
        }
        nodes = []
        cmd = ("sudo salt '*' pillar.item {} --out yaml --static 2>/dev/null"
               .format(' '.join(_map.values() + ['_param:pxe_admin_address'])))
        ...
        for node_name in yaml_output.keys():
            ...
            node = manager.Node(
                ...
                roles=[x for x in _map if yaml_output[node_name][_map[x]]],

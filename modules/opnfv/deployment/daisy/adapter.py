        ...
        for cluster in clusters:
            if options and 'cluster' in options and options['cluster']:
                if cluster["id"] != options['cluster']:
                    continue
            cmd = 'source /root/daisyrc_admin; daisy host-list --cluster-id {} | grep -v "+--"'.format(cluster["id"])
            output = self.installer_node.run_cmd(cmd)
            lines = output.rsplit('\n')
            if len(lines) < 2:
                logger.info("No nodes found in the cluster {}".format(cluster["id"]))
                continue
            ...
            for i in range(1, len(lines)):
                ...
                cmd_get_role = 'source /root/daisyrc_admin; daisy host-detail {} | grep "^| role"'.format(id)
                output_role = self.installer_node.run_cmd(cmd_get_role)
    ...
    def get_sdn_version(self):
        ...
        for node in self.nodes:
            if manager.Role.CONTROLLER in node.roles and node.is_active():
                cmd_get_containers = 'docker inspect --format=\'{{.Name}}\' `docker ps -q`'
                output = node.run_cmd(cmd_get_containers)
                if '/opendaylight' in output.rsplit('\n'):
                    cmd = 'docker exec opendaylight sudo yum info opendaylight 2>/dev/null | grep Version | tail -1'
...
class DaisyNode(manager.Node):
    ...
    def __init__(self,
                 ...
                 ssh_client=None,
                 info=None):
        super(DaisyNode, self).__init__(id, ip, name, status, roles, ssh_client, info)
    ...
    def is_odl(self):
        ...
        if manager.Role.CONTROLLER in self.roles and self.is_active():
            cmd_get_containers = 'docker inspect --format=\'{{.Name}}\' `docker ps -q`'
            output = self.run_cmd(cmd_get_containers)
            if '/opendaylight' in output.rsplit('\n'):
                return True
        return False
    ...
    def get_ovs_info(self):
        ...
        '''
        if self.is_active():
            cmd = 'docker exec openvswitch_vswitchd ovs-vsctl --version | head -1 | awk \'{print $NF}\''
